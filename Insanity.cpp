...
Function * Insanity::generateHailStone(Instruction * I){
	...
	Constant * c = mod -> getOrInsertFunction("hailstone", 
    ...
    Value * space = builder -> CreateAlloca(
            Type::getInt32Ty(mod -> getContext()));
    ...
    Value * store = builder -> CreateAdd(
            builder -> CreateMul(oddChecker, three), one);
...
Value * Insanity::obfuscateBr(BranchInst * I, Value * extra, 
    ...
    if (I -> isConditional()){
        ...
        BasicBlock * trap2 = BasicBlock::Create(I -> getContext(), 
                "trap2", I -> getFunction());
